...
(function () {
    ...
    showModal(`
      <div style="padding:30px; position:relative;">
        ...
        <div style="display:flex; flex-direction:column; gap:12px;">
          ...
          <a class="glass glass-hover" href="mailto:${esc(DATA.ticketEmail)}" style="padding:16px; display:flex; align-items:center; gap:14px; text-decoration:none;"><i class="fa-solid fa-envelope" style="color:var(--accent); font-size:18px;"></i><span><span style="display:block; font-weight:600;">${esc(DATA.ticketEmail)}</span><span style="display:block; color:var(--faint); font-size:12.5px;" class="mono">Email opens a ticket automatically</span></span></a>
        </div>
      </div>
    `, 520);
  ...
  function showTicketSent(to, href, portal) {
    showModal(`
      <div style="padding:36px 30px; text-align:center;">
        ...
        <p style="color:var(--muted); font-size:14.5px; line-height:1.6; margin:0 auto 12px; max-width:48ch;">Your email app should open with the ticket pre-filled and addressed to <span class="mono" style="color:var(--accent);">${esc(to)}</span>. Review it and hit send — Spiceworks opens a ticket automatically and emails you the ticket number.</p>
  ...
  function showKudos(presetId) {
    ...
    document.getElementById('kd-email').addEventListener('click', () => {
      ...
      window.location.href = `mailto:${c.r.email}?subject=${encodeURIComponent(c.subject)}&body=${encodeURIComponent(c.body)}`;
